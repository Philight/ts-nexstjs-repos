...
export default class UserService {
  ...
  static async toggleFavorite(productId: number): Promise<AxiosResponse<User>> {
    return $api.patch<User>(`/${UserService.controller}/profile/products/favorite/${productId}`);
  }
  static async toggleHelpful(reviewId: number): Promise<AxiosResponse<User>> {
    return $api.patch<User>(`/${UserService.controller}/profile/reviews/helpful/${reviewId}`);
  }
